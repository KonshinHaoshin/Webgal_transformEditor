/**
 * 从 JSONL 文件中提取 motions 和 expressions 列表
 * 使用后端 Rust 代码读取文件，避免路径转换问题
 * @param jsonlPath JSONL 文件路径（相对路径，相对于游戏文件夹的 figure 目录）
...
export async function extractMotionsAndExpressions(
  ...
  try {
    ...
    console.log(`🔍 正在通过后端加载 JSONL: ${jsonlPath}`);
