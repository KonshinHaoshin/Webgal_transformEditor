...
import { BevelFilter } from '../filters/BevelFilter.ts';
import * as PIXI from 'pixi.js';
import { BlurFilter } from '@pixi/filter-blur';
import { INIT_RAD, RadiusAlphaFilter } from '../filters/RadiusAlphaFilter';
...
interface PropertyConfig {
    ...
    defaultValue: number;
    isBoolean?: boolean;
    overrideSet?: (value: number, filter: PIXI.Filter, container: PixiContainer) => void;
    overrideGet?: (filter: PIXI.Filter | undefined, defaultValue: number, container: PixiContainer) => number;
}
...
export class PixiContainer extends PIXI.Container {
    ...
    private _getPropertyValue(propertyName: string): number {
        ...
        if (!propConfig) {
            console.warn(`PixiContainer: Unknown property configuration for getter: ${propertyName}`);
            return 0;
        }
    ...
    private _setPropertyValue(propertyName: string, value: number): void {
        const propConfig = PROPERTY_CONFIGS[propertyName];
        if (!propConfig) {
            console.warn(`PixiContainer: Unknown property configuration for setter: ${propertyName}`);
            return;
        }
        if (propConfig.isBoolean) {
        ...
        if (propConfig.overrideSet) {
            ...
            console.warn(
                `PixiContainer: Property '${propertyName}' has neither overrideSet nor filterProperty defined for value setting.`,
            );
        }
