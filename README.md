...
2. **选择立绘**：
   - 点击 "展开" 按钮展开文件列表
   - 在 "立绘文件" 列表中点击任意立绘文件名即可加载

3. **选择背景**：
   - 在 "背景文件" 列表中点击任意背景文件名即可加载
...
1. **精确对齐**：使用辅助线功能可以帮助你将模型精确对齐到构图关键位置

2. **色彩检查**：在最终导出前，使用观测层的"颜色"模式检查整体配色是否和谐

3. **明度层次**：使用观测层的"明度"模式检查场景的明暗分布，确保主体突出

4. **批量编辑**：使用多选功能可以同时移动、缩放或旋转多个模型

5. **WebGAL 模式**：如果你在制作 WebGAL 游戏，强烈建议使用 WebGAL 模式，可以更直观地管理游戏资源

6. **滤镜组合**：可以叠加多个滤镜效果，创建更丰富的视觉效果
